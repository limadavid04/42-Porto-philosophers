...
/*   Updated: 2024/01/16 11:13:32 by dlima            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_starvation(t_philo *philo)
{
	...
	interval = get_current_time_millis() - philo->last_meal;
...
void	monitor_philos(t_philo *philos, t_data *data)
{
	...
	while (1)
	{
		ate_min_meals = 0;
		i = 0;
		while (i < data->nbr_philos && check_if_dead(&philos[i]) != 1)
		{
		...
		if (get_dead_flag_val(data) == 1)
			break ;
		if (ate_min_meals == data->nbr_philos && data->meal_minimum != -1)
		{
			pthread_mutex_lock(&data->dead_mutex);
			data->dead_flag = 1;
			pthread_mutex_unlock(&data->dead_mutex);
